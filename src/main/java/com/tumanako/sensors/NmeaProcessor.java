...
public class NmeaProcessor implements GpsStatus.NmeaListener, IDroidSensor, IDashMessages
{
  ...
  /** NMEAData Time (UTC), HHMMSS (for example 123542.0 for 12:35:42 pm) */
  private float  gpsTime   = 0f;
  /** NMEAData Latitude (Decimal Degrees) */
  private double gpsLat    = 0.0;
  /** NMEAData Longitude (Decimal Degrees) */
  private double gpsLon    = 0.0;
  /** NMEAData Fix Quality * */
  private int    gpsQual   = 0;
  /** NMEAData Number of satellites */
  private int    gpsSats   = 0;
  /** NMEAData Altitude (m) */
  private float  gpsAlt    = 0f;
  /** NMEAData Ground track (Deg, True) */
  private float  gpsTrackT = 0f;
  /** NMEAData Grong speed (kph) */
  private float  gpsSpeed  = 0f;
  /** Will store the last GGA and */
  private String gpsLastGGA = "";
  /** VTG strings received (for debugging) */
  private String gpsLastVTG = "";

  // ***** Information derived during operation: **********
  /**
   * Set to true after the 'GSA' sentence arrives (last in a cycle),
   * and false when any other sentence arrives.
   */
